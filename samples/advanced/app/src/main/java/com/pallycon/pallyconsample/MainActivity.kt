...
class MainActivity : AppCompatActivity() {
    ...
    private val pallyConEventListener: PallyConEventListener = object : PallyConEventListener {
        ...
        override fun onFailed(currentUrl: String?, e: PallyConLicenseServerException?) {
            ...
            e?.let {
                Toast.makeText(this@MainActivity, "Server Error - ${it.errorCode()}, ${it.message()}", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun initializeList() {
        ...
        wvSDK2.setPallyConEventListener(pallyConEventListener)
        ...
        wvSDK3.setPallyConEventListener(pallyConEventListener)
    ...
    private fun prepareForIndex(index: Int) {
        CoroutineScope(Dispatchers.Main).launch {
            ...
            try {
                contents[index].wvSDK.migrateDownloadedContent(
                    url = contents[index].content.url!!, // remote content URL
                    cid = contents[index].cid, // ID of content
                    siteId = contents[index].content.drmConfig!!.siteId!!, // inputs Site ID which is issued on PallyCon service registration
                    ...
                    0 -> {
                        scope.launch {
                            wvSDK.downloadLicense({
                                ...
                                Toast.makeText(this@MainActivity, "${e.msg}", Toast.LENGTH_SHORT).show()
                                print(e.msg)
                            })
                        }
                    }
