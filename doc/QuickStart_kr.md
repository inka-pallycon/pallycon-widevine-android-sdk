...
5. AndroidManifest.xml 에 다운로드 Service를 등록합니다. (샘플 소스 참조)
```xml
<service
...
6. MainActivity에 PallyConEventListener를 구현합니다. (샘플 소스 참조)
...
7. 다운로드할 콘텐츠 정보를 넣어 PallyConWvSDK 객체를 생성합니다. (샘플 소스 참조)
...
val wvSDK = PallyConWvSDK.createPallyConWvSDK(
    Context, // Context
    data// add PallyConEventListener
)

wvSDK.setPallyConEventListener(pallyConEventListener)
```

8. 다운로드할 콘텐츠의 트랙 정보를 가져옵니다. (샘플 소스 참조)
```kotlin
...
9. 트랙 정보에서 다운로드할 트랙을 선택합니다. (샘플 소스 참조)
```kotlin
...
10. 콘텐츠가 이미 다운로드되어 있는지 확인 후 다운로드를 실행합니다. (샘플 소스 참조)
...
11. 다운로드된 콘텐츠를 재생하려면 다음 API를 사용하여 MediaItem 또는 MediaSource 를 획득합니다. (샘플 소스 참조)
```kotlin
...
12. PlayerActivity.kt에 다음 개발 가이드를 참고해 플레이어를 구현합니다.
    http://google.github.io/ExoPlayer/guide.html

> Exoplayer에 관한 자세한 정보는 다음 구글 문서를 참고하시기 바랍니다.
> https://developer.android.com/guide/topics/media/exoplayer.html

11. DRM 라이선스의 license duration 와 playback duration 을 획득합니다.
```kotlin
...
12. ExoPlayer 를 다음과 같이 설정합니다. (샘플 소스 참조)
```kotlin
